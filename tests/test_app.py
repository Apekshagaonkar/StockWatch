...
def stock_api():
    ...
    api = StockAPI()
    api.cache["APPL"] = (
        MOCK_STOCK_DATA  # Simulated Cache hit for Mock functionality test
    )
    return api
...
def test_lookup_valid_date_cached(stock_api):
    """Test lookup function for a valid date for data in cache."""
...
def test_lookup_invalid_date_cached(stock_api):
    """Test lookup function with a date not in the dataset present in cache."""
...
def test_get_min_valid_range_cached(stock_api):
    """Test get_min function with a valid range for data in cache."""
...
def test_get_min_invalid_range_cached(stock_api):
    """Test get_min function with an invalid range (greater than available data) for data in cache."""
    with pytest.raises(HTTPException) as excinfo:
...
def test_get_min_negative_range_cached(stock_api):
    """Test get_min function with a negative range for data in cache."""
    with pytest.raises(HTTPException) as excinfo:
...
def test_get_max_valid_range_cached(stock_api):
    """Test get_max function with a valid range for data in cache."""
...
def test_get_max_invalid_range_cached(stock_api):
    """Test get_max function with an invalid range (negative or zero) for data in cache."""
    with pytest.raises(HTTPException) as excinfo:
...
def test_get_max_negative_range_cached(stock_api):
    """Test get_max function with a negative range for data in cache."""
    with pytest.raises(HTTPException) as excinfo:
...
def test_lookup_valid_date(stock_api):
    """Test lookup function for a valid date for data not in cache."""
...
def test_cache_hit(stock_api):  # Actual Cache hit for end to end test
    """Test cache hit."""
    stock_api.get_min("IBM", 2)
    assert stock_api.cache_hit == 0
    stock_api.get_max("IBM", 2)
    assert stock_api.cache_hit == 1
...
def test_get_min_valid_range(stock_api):
    ...
    assert "min" in result and type(result["min"]) is float
...
def test_get_max_valid_range(stock_api):
    ...
    assert "max" in result and type(result["max"]) is float
