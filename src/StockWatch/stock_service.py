...
class StockAPI:
    ...
    def _fetch_stock_data(self,symbol: str, outputsize: str = "compact"):
        ...
        if "Information" in data and "our standard API rate limit is 25 requests per day" in data["Information"]:
            raise HTTPException(status_code=429, detail="API rate limit exceeded")

        if "Time Series (Daily)" not in data:
            raise HTTPException(status_code=400, detail="Invalid symbol or API limit exceeded")
    ...
    def lookup(self, symbol: str, date: str):
        ...
        # Check if the date is available in the stock data (market closure handling/ Date not in last 100 days / Future date)
        if date not in stock_data:
            raise HTTPException(status_code=404, detail="Data not available for the given date")
    ...
    def get_min(self, symbol: str, n: int):
        ...
        if n <= 0:
            raise HTTPException(status_code=400, detail="Range `n` must be greater than 0")
        ...
        if n > len(all_dates):
            raise HTTPException(status_code=400, detail="Requested range `n` exceeds available data")
